...
object Day20 extends App {
  ...
  def madd(idx: Long, delta: Long, mod: Int): Int = ((idx + delta % mod + mod) % mod).toInt

  def mix(xs: Vector[(Long, Int)]): Vector[(Long, Int)] = {
    (0 until xs.length).foldLeft(xs) { (acc, id) =>
      val idx = acc.indexWhere(_._2 == id)
      val newIdx = madd(idx, acc(idx)._1, acc.length - 1)
  ...
  def groveCoords(xs: Vector[(Long, Int)]): Long = {
    ...
    List(1000, 2000, 3000).map { i => xs(madd(idx0, i, xs.length))._1 }.sum
  }
